...
def slice_layers(systems):
    bound = 300     # +/- boundary in ly relative to Sol
    step = 10       # Step size in ly
    lower = -bound
    ...
    while (lower < bound):
        ...
        if (lower < 0):
            ...
            fname = 'p'
        fname += '{:03d}'.format(abs(lower)) + '.json'
        f = open (fname, "w")
        for s in systems:
            if ((s['y'] >= lower) & (s['y'] < upper)):
                f.write(json.dumps(s) + '\n')
        ...
        f_summary.write ('{:04d}'.format(lower) + ',')
        f_summary.write (str(stats['n']) + ',')
        f_summary.write (str(stats['fed']) + ',')
        f_summary.write (str(stats['imp']) + ',')
        f_summary.write (str(stats['all']) + ',')
...
#    slice_regions(systems)
    slice_layers(systems)
